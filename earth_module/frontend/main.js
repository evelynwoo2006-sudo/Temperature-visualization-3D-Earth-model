...
const LANDING_CAMERA_DISTANCE = 5.0;
let lastLandingMode = null;
...
function formatCountryZhWithCity(countryZh, city, countryCode) {
  const base = String(countryZh || "—").trim() || "—";
  if (base !== "—") return base;
...
async function init() {
  ...
  renderer.setSize(Math.max(1, initRect.width), Math.max(1, initRect.height), false);
  renderer.outputColorSpace = THREE.SRGBColorSpace;

  scene = new THREE.Scene();

  camera = new THREE.PerspectiveCamera(45, Math.max(1, initRect.width) / Math.max(1, initRect.height), 0.1, 200);
...
function animate() {
  ...
  if (orbit && !isFocusing) {
    const landing = !!document.body && document.body.classList.contains("is-landing");
    if (lastLandingMode !== landing) {
      lastLandingMode = landing;
      const dist = landing ? LANDING_CAMERA_DISTANCE : CONFIG.cameraDistance;
